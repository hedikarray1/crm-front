...
export const GetAllCustomers = (per_page, page) => {
  return async (dispatch) => {
    ...
    try {
      axios.get(WP_JSON + "wc/v3/customers?consumer_key=" + token.key + "&consumer_secret=" + token.secret + "&per_page=" + per_page + "&page=" + page, {
...
export const DeleteCustomer = (id,per_page,page) => {
  return async (dispatch) => {
    ...
    try {
      axios.delete(WP_JSON + "wc/v3/customers/"+id+"?force=true&consumer_key=" + token.key + "&consumer_secret=" + token.secret , {
...
export const AddCustomer = (customer) => {
  return async (dispatch) => {
    ...
    try {
      axios.post(WP_JSON + "wc/v3/customers?consumer_key=" + token.key + "&consumer_secret=" + token.secret ,customer)
        .then((response) => {
...
export const UpdateCustomer = (id,customer) => {
  return async (dispatch) => {
    ...
    try {
      axios.put(WP_JSON + "wc/v3/customers/"+id+"?consumer_key=" + token.key + "&consumer_secret=" + token.secret ,customer)
        .then((response) => {
          console.log("add customer data :", response);
          dispatch({
